...
func TestReservationPost(t *testing.T) {
	...
	localDTI.CreateValue = false
	create := &backend.Reservation{Addr: net.ParseIP("1.1.1.1"), Token: "kfred"}
...
func TestReservationPut(t *testing.T) {
	...
	localDTI.UpdateValue = false
	update := &backend.Reservation{Addr: net.ParseIP("1.1.1.1"), Token: "kfred"}
	...
	localDTI.UpdateValue = true
	update = &backend.Reservation{Addr: net.ParseIP("1.1.1.2"), Token: "kfred"}
	localDTI.UpdateError = nil
	v, _ = json.Marshal(update)
	req, _ = http.NewRequest("PUT", "/api/v3/reservations/1.1.1.1", strings.NewReader(string(v)))
	req.Header.Set("Content-Type", "application/json; charset=utf-8")
