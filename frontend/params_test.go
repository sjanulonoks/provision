...
func TestParamPost(t *testing.T) {
	...
	localDTI.CreateValue = &backend.Param{Name: "fred", Value: "kfred"}
...
func TestParamGet(t *testing.T) {
	...
	localDTI.ValidateContentType(t, "application/json; charset=utf-8")
	localDTI.ValidateError(t, "API_ERROR", "parameters GET: fred: Not Found")

	localDTI.GetValue = &backend.Param{Name: "fred", Value: "kfred"}
...
func TestParamPut(t *testing.T) {
	...
	localDTI.UpdateValue = &backend.Param{Name: "fred", Value: "kfred"}
	...
	localDTI.UpdateValue = &backend.Param{Name: "kfred", Value: "kfred"}
	localDTI.UpdateError = nil
	v, _ = json.Marshal(localDTI.UpdateValue)
	req, _ = http.NewRequest("PUT", "/api/v3/params/fred", strings.NewReader(string(v)))
	req.Header.Set("Content-Type", "application/json; charset=utf-8")
	...
	localDTI.ValidateCode(t, http.StatusBadRequest)
	localDTI.ValidateContentType(t, "application/json; charset=utf-8")
	localDTI.ValidateError(t, "API_ERROR", "parameters PUT: Key change from fred to kfred not allowed")
	...
	var be backend.Param
	json.Unmarshal(w.Body.Bytes(), &be)
	if !reflect.DeepEqual(be.Value, "kfred") {
		t.Errorf("Returned Param was not correct: %v %v\n", "kfred", be.Value)
	}
}
...
func TestParamDelete(t *testing.T) {
	...
	localDTI.RemoveValue = &backend.Param{Name: "fred", Value: "kfred"}
