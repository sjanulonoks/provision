...
VM_DIR="$HOME/.cache/dr-provision/vms"
...
run_kvm() {
    ...
    local kvmargs=(-enable-kvm
        ...
        -name "kvm-$vm_gen")
    if [[ $uefi = true ]]; then
        # curent arch linux only for now.  Different distros may put their
        # ovmf firmware in different places.
        [[ -f $VM_DIR/uefi_vars.bin ]] || \
            cp /usr/share/ovmf/x64/OVMF_VARS.fd "$VM_DIR/uefi_vars.bin"
        kvmargs+=(-drive if=pflash,format=raw,readonly,file=/usr/share/ovmf/x64/OVMF_CODE.fd)
        kvmargs+=(-drive if=pflash,format=raw,file="$VM_DIR/uefi_vars.bin")
    fi
